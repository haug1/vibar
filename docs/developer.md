...
- `playerctl` reads media metadata/status from MPRIS over DBus (`zbus`), updates on DBus signals (`NameOwnerChanged` + `PropertiesChanged`), supports placeholder-based output formatting, and optionally exposes a GTK popover for transport controls + `SetPosition` seeking.
