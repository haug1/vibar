...
- PulseAudio module uses native `libpulse` subscriptions/introspection (`src/modules/pulseaudio.rs`) rather than shelling out to `pactl`, and now supports an optional popover controls UI (default sink mute/volume, active sink-input stream mute/volume, and default-sink port switching).
