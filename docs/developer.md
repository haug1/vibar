...
- `playerctl` reads media metadata/status from MPRIS over DBus (`zbus`), updates on DBus signals (`NameOwnerChanged` + `PropertiesChanged`) plus a lightweight periodic snapshot for position freshness, supports placeholder-based output formatting, and optionally exposes a GTK popover for transport controls + `SetPosition` seeking with progress timestamps.
