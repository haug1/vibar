...
- `playerctl` reads media metadata/status from MPRIS over DBus (`zbus`), updates on DBus signals (`NameOwnerChanged` + `PropertiesChanged`) plus a lightweight periodic snapshot for position freshness, supports placeholder-based output formatting, optional fixed-width clipping (`fixed-width`) with configurable marquee mode (`marquee`: `off`/`hover`/`open`/`always`) for long metadata text, and optionally exposes a GTK popover with top-aligned transport controls, a two-column key/value metadata grid, and `SetPosition` seeking with progress timestamps (hover tooltip is suppressed while controls are open to avoid overlap; controls popover width tracks module width and wraps long values).
